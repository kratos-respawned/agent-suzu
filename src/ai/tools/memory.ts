...
export const MemoryTools = tool({
  ...
  execute: async ({ knowledge, action, key }, { messages, toolCallId }) => {
    switch (action) {
      ...
      case "clear":
        return clearConversation.execute && await clearConversation.execute({}, { messages, toolCallId });
      case "reset":
        return resetModel.execute && await resetModel.execute({}, { messages, toolCallId });
    }
  },
});
